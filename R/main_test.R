...
main_test <- function(label, num_items, offset = 1) {
  ...
  for(item_id in (offset + 1):(offset + num_items)){
    label <- sprintf("q%d", item_id)
    prompt <- get_prompt(item_id, num_items, sprintf("TDAC_000%d_PROMPT", item_id))
    choices <- sprintf("btn%d_text", 1:num_options)
    choice_ids <- sprintf("TDAC_000%d_CHOICE%d", item_id, 1:num_options)
    labels <- c()
    for(i in choice_ids){
      labels <- c(labels, psychTestR::i18n(i))
    }
    item_page <- psychTestR::NAFC_page(label = label,
                        prompt = prompt,
                        choices = choices,
                        labels = labels)
    elts <- c(elts, item_page)
  }
  psychTestR::new_timeline(elts, dict = DAC::DAC_dict)
}
